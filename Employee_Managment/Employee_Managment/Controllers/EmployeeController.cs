...
namespace Employee_Managment.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        public async Task<IActionResult> Index(DateTime? selectedDate)
        {
            ...
            var firstDayOfMonth = new DateTime(selectedDate.Value.Year, selectedDate.Value.Month, 1);
            var lastDayOfMonth = firstDayOfMonth.AddMonths(1).AddDays(-1);

            var employees = await _employeeRepository.GetEmployeesForDateRangeAsync(firstDayOfMonth, lastDayOfMonth);
        ...
        [HttpPost]
        public async Task<IActionResult>  CreatePunch(PunchEvent punchEvent)
        {
            if (ModelState.IsValid)
            {
                await _employeeRepository.CreatePunchEventAsync(punchEvent);
